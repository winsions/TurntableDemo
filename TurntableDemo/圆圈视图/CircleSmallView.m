...
+(CircleSmallView *)createSmallCircleView{
    ...
    UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, smallView.bounds.size.width, smallView.bounds.size.height)];
    [btn addTarget:smallView action:@selector(test) forControlEvents:UIControlEventTouchUpInside];
...
//    [self setTintColor:[ZJFactoryMethod colorWithHexString:model.fontColor]];
    self.backgroundColor = [ZJFactoryMethod colorWithHexString:model.baseViewColor];
}
...
-(void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isStartTurntable) {
        ...
        CGPoint previousPoint = [self relativePoint:previousLocation and:self.superview.center];
        ...
        CGPoint nowPoint = [self relativePoint:nowLocation and:self.superview.center];
